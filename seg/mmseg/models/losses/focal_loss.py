...
def py_sigmoid_focal_loss(pred,
    ...
    if weight is not None:
        ...
        assert weight.dim() == loss.dim(), f'weight dim: {weight.dim()}, loss dim: {loss.dim()}'
...
def sigmoid_focal_loss(pred,
    ...
    if weight is not None:
        weight = weight.float()
        if weight.shape != loss.shape and weight.shape[1]*weight.shape[2] == loss.shape[0]:          # and weight.size(0) == loss.size(0):
            # For most cases, weight is of shape (N, ),
            # which means it does not have the second axis num_class
            # weight = weight.view(-1, 1)
            weight = weight.reshape(-1,1)
        assert weight.dim() == loss.dim(), f'weight dim: {weight.dim()}, {weight.shape}, loss dim: {loss.dim()}, {loss.shape}'
        final_weight = final_weight * weight
    if class_weight is not None:
        final_weight = final_weight * pred.new_tensor(class_weight)
    if valid_mask is not None:
